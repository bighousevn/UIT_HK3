...
insert into SANPHAM(MASP,TENSP,DVT,NUOCSX,GIA) values --SANPHAM--
...
insert HOADON (SOHD,NGHD,MAKH,MANV,TRIGIA) values --HOADON--
...
insert into CTHD(SOHD,MASP,SL) values --CTHD--
('1001', 'TV02', 10),
('1001', 'ST01', 5),
('1001', 'BC01', 5),
('1001', 'BC02', 10),
...
WHERE (NGDK < '2007-01-01' AND DOANHSO >= 10000000)
 ...
 -------------Câu 11--------------
 ALTER TABLE HOCVIEN
 ADD CONSTRAINT CK_HS CHECK (GETDATE() - NGSINH >= 18)

-------------Câu 12--------------
  ALTER TABLE GIANGDAY
  ADD CONSTRAINT CK_GIANGDAY CHECK (DENNGAY > TUNGAY)

-------------Câu 13--------------
ALTER TABLE GIAOVIEN
ADD CONSTRAINT CK_GIAOVIEN CHECK (GETDATE() - NGSINH >=22)

-------------Câu 14--------------
ALTER TABLE MONHOC
ADD CONSTRAINT CK_TINCHI CHECK (ABS (TCLT - TCTH) BETWEEN 0 AND 3)
...
SELECT SOHD,TRIGIA
FROM HOADON
WHERE MONTH(NGHD) = 1 AND YEAR(NGHD) = 2006
ORDER BY NGHD ASC, TRIGIA DESC

-------------Câu 8--------------
SELECT KHACHHANG.MAKH,HOADON.MAKH, HOTEN
FROM KHACHHANG, HOADON
WHERE HOADON.NGHD='2007-01-01' AND KHACHHANG.MAKH = HOADON.MAKH

-------------Câu 9--------------
SELECT SOHD,TRIGIA
FROM HOADON, NHANVIEN
WHERE HOADON.MANV = NHANVIEN.MANV AND NHANVIEN.HOTEN = 'Nguyen Van B'
...
AND SOHD IN (
	SELECT SOHD 
	FROM KHACHHANG, HOADON
	WHERE (HOADON.MAKH = KHACHHANG.MAKH) AND (KHACHHANG.HOTEN = 'Nguyen Van A') AND YEAR(NGHD) = 2006 AND MONTH(NGHD)=10
)

-------------Câu 11--------------
SELECT SOHD 
FROM SANPHAM, CTHD
WHERE SANPHAM.MASP = CTHD.MASP AND (SANPHAM.MASP ='BB01' OR SANPHAM.MASP ='BB02')
