...
GO
CREATE TRIGGER CAU4 ON DOTCN
FOR INSERT, UPDATE
AS
BEGIN
	IF EXISTS ( SELECT *
			FROM INSERTED I, GIONGVN GI
			WHERE I.MAGIONG=GI.MAGIONG AND GI.LOAIVN='Bo' AND MONTH(I.NGXUATDK)-MONTH(I.NGBD)<12)
		BEGIN
...
--5.Tìm các giống vật nuôi (MAGIONG, TENGIONG) thuộc loại “Bo” được chăn nuôi theo phương thức “thả tự do” dự kiến xuất trong tháng 12/2024
SELECT GVN.MAGIONG,TENGIONG
FROM DOTCN DOT,GIONGVN GVN
WHERE DOT.MAGIONG=GVN.MAGIONG AND LOAIVN='Bo' AND PHGTHUC = 'thả tự do' AND month(NGXUATDK)=12
...
WHERE NOT EXISTS
		(SELECT CS.MACS,TENCS
			FROM GPCN GP,CSCHANNUOI CS, GIONGVN GVN, CTGP CT
			WHERE CS.MACS = GP.MACS AND GP.MAGP=CT.MAGP AND CT.MAGIONG = GVN.MAGIONG AND GVN.LOAIVN ='De') 
AND EXISTS
	(SELECT * FROM DOTCN 
		WHERE CS.MACS=DOTCN.MACS AND YEAR(NGBD)=2024 AND YEAR(NGXUATDK)=2024)


--7. Tính thời gian nuôi thực tế trung bình (theo đơn vị tháng) của từng loại vật nuôi
--theo từng hình thức chăn nuôi (1đ).
SELECT	LOAIVN, PHGTHUC, AVG(year(NGXUATDK)*12+month(NGXUATDK)-year(NGBD)*12-month(NGBD))
FROM DOTCN, GIONGVN
...
SELECT LOAIVN
FROM GIONGVN GI1
WHERE GI1.MAGIONG IN (SELECT TOP 1 WITH TIES GI2.MAGIONG
						FROM GIONGVN GI2, CTGP, GPCN
						WHERE GI2.MAGIONG = GI1.MAGIONG AND GI2.MAGIONG = CTGP.MAGIONG AND CTGP.MAGP=GPCN.MAGP AND YEAR(GPCN.NGCAP)=2024
						GROUP BY GI2.MAGIONG, CTGP.SL
						ORDER BY SUM(CTGP.SL) DESC)
...
WHERE NOT EXISTS (SELECT *
				...
				WHERE LOAIVN='Bo' AND NOT EXISTS (SELECT *
												FROM CTGP, GPCN
												WHERE CTGP.MAGIONG=GIONGVN.MAGIONG AND GPCN.MACS=CSCHANNUOI.MACS AND CTGP.MAGP=GPCN.MAGP AND CTGP.SL>=20))
